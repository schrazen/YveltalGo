...
- **YOLO Neural Network Solver**: Integrated object-detection model (`Solver100k.pt`) that auto-crops, removes noise, and predicts captcha digit challenges.
...
2. Open `PokeGrinder/config.json` in your editor and configure your accounts:
   ...
   ```
> [!IMPORTANT]
> Keep your real tokens **only** in `config.json`. The repository's `.gitignore` is configured to prevent `config.json` and runtime logs from ever being tracked by git.
